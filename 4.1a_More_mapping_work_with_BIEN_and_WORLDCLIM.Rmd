...
#Spain! It's the Iberian penninsula
2. Is altitude a climate variable? How do the peninsula maps plotted above support your answer?
#Yes! The temperature is significantly colder and higher altitudes. 
...
library("spocc")
library("taxize")
results=gnr_resolve(names="Minuartia rossii")
head(results$matched_name)
results=gnr_resolve(names="Petrocoptis hispanica")
head(results$matched_name)
```
...
```{r}
name="Petrocoptis hispanica"  
tsn=get_tsn(name, accepted=FALSE)
tsn
lapply(tsn, itis_acceptname)
```
#looks like Petrocoptis hispanica is not here 
```{r}
tax_name(q="Petrocoptis hispanica", get="family", db="ncbi")
```

```{r}
downstream("Caryophyllaceae", downto="species", db="itis")
```

```{r}
name="Minuartia rossii"  
tsn=get_tsn(name, accepted=FALSE)
tsn
lapply(tsn, itis_acceptname)
```
...
#no subspecies for Minuartia rossii

```{r}
tax_name(q="Minuartia rossii", get="subspecies", db="ncbi")
```
...
#data frame for Petrocoptis hispanica
```{r}
Flower <- occ(q="Petrocoptis hispanica", from="gbif", limit=50)
Flower
Flower.dat <- (Flower$gbif$data)
head(Flower.dat)
Flower.df <- occ2df(Flower)
Flower.df
dim(Flower.df)
Flower.df <- Flower.df[complete.cases(Flower.df),]
dim(Flower.df)
Flower.df
```

#data frame for Minuartia rossii
```{r}
Planty <- occ(q="Minuartia rossii", from="gbif", limit=50)
Planty
Planty.dat <- (Planty$gbif$data)
head(Planty.dat)
Planty.df <- occ2df(Planty)
Planty.df
dim(Planty.df)
Planty.df <- Planty.df[complete.cases(Planty.df),]
dim(Planty.df)
Planty.df
```
...
#Searched all of the names, clean! 
...
#for Minuartia rossii
```{r}
P_hispanica <- BIEN_occurrence_species(species = "Minuartia rossii")

P_hispanica_rangemap <- BIEN_ranges_load_species("Minuartia rossii")

map('world', fill=T, col="grey", bg="light blue", xlim=c(-180,-20), ylim=c(-60, 80))
plot(P_hispanica_rangemap, col="green", add=T)
```
...
#here is the climate/rangemap for Minuartia rossii

```{r}
plot(all.worldclim[["bio1"]])
points(P_hispanica[,c("longitude", "latitude")], pch = 24, cex = 0.5)
plot(all.worldclim[["bio12"]])
points(P_hispanica[,c("longitude", "latitude")], pch = 16, cex = 0.5, col = "red")


```

```{r}
plot(all.worldclim[["bio1"]], xlim=c(-160,-130),ylim=c(50,75))
```

#here is my cropped rangemap/climate map for Minuartia rossii
```{r}
plot(all.worldclim[["bio1"]], xlim=c(-160,-130),ylim=c(50,75))
points(P_hispanica[,c("longitude", "latitude")], pch = 24, cex = 0.5)
plot(all.worldclim[["bio12"]])
...
#I am pretty convinced that Minuartia rossii is endemic to this climate/range as it is found nowhere else and the climate is fairly uniform throughout. 
...
Thespesia_grandiflora <- BIEN_occurrence_species(species = "Thespesia grandiflora")
```

```{r}
str(Thespesia_grandiflora)
head(Thespesia_grandiflora)
tail(Thespesia_grandiflora)
```

```{r}
Thespesia_grandiflora_rangemap <- BIEN_ranges_load_species("Thespesia grandiflora")
```

```{r}
map('world', fill=T, col="grey", bg = "light blue", xlim=c(-180,20),ylim=c(-60,80))
plot(Thespesia_grandiflora_rangemap, col="green", add=T)
```

```{r}
map('world', fill=T, col="grey", bg="light blue", xlim=c(-180,-20), ylim=c(-60, 80))
plot(Thespesia_grandiflora_rangemap, col="green", add=T)
points(cbind(Thespesia_grandiflora$longitude, Thespesia_grandiflora$latitude), col="blue", pch=20, cex=1)
```

```{r}
plot(all.worldclim[["bio1"]])
points(Thespesia_grandiflora[,c("longitude", "latitude")], pch = 24, cex = 0.5)
plot(all.worldclim[["bio12"]])
points(Thespesia_grandiflora[,c("longitude", "latitude")], pch = 16, cex = 0.5, col = "red")


```

```{r}
plot(all.worldclim[["bio1"]], xlim=c(-100,-50),ylim=c(10,30))
```

#here is the cropped climate/rangemap from the hibiscus
...
#for Minuartia rossii, I was primarily focusing on Alaska
#for the hibiscu, Puerto Rico! 